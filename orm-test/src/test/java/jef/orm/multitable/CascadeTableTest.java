...
@DataSourceContext({ @DataSource(name = "hsqldb", url = "${hsqldb.url}", user = "sa", password = ""), @DataSource(name = "oracle", url = "${oracle.url}", user = "${oracle.user}", password = "${oracle.password}"),
		@DataSource(name = "mysql", url = "${mysql.url}", user = "${mysql.user}", password = "${mysql.password}"), @DataSource(name = "postgresql", url = "${postgresql.url}", user = "${postgresql.user}", password = "${postgresql.password}"),
		@DataSource(name = "derby", url = "${derby.url}"), @DataSource(name = "sqlite", url = "jdbc:sqlite:test.db?date_class=integer&date_string_format=yyyy-MM-dd HH:mm:ss"),
...
public class CascadeTableTest extends MultiTableTestBase {
	...
	public void testMerge() throws SQLException {
		{
			Query<Person> query = new PKQuery(MetaHolder.getMeta(Person.class), 3);
			Person p = db.load(query, false);
		...
		{
			Query<Person> query = QB.create(Person.class).addCondition(Person.Field.id.eq(3));
	...
	public void testConditionInOneObj() throws SQLException {
		...
		Person q = new Person();
		q.getQuery().addCondition(Person.Field.age.gt(16));
		q.getQuery().addCondition(School.Field.id.eq(2));// 凡是引用一个其他表的条件要用RefField包裹
	...
	public void testOrAndConditionWithDiffQueryObj() throws SQLException {
		...
		Or or = new Or();
		or.addCondition(and);
		or.addCondition(Person.Field.age.ge(99));
