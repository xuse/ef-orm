...
public class CaseDatabaseMetadata {
	...
	/**
	 * 删除表并建表。 删除表时会主动删除相关的所有主外键、索引、约束。 建表时会同时创建表的索引。
	 * 
	 * @throws SQLException
	 */
	@Test
	...
	public void testIndexes() throws SQLException {
		...
		idef.setUnique(true);
		Index index1 = Index.valueOf(idef, MetaHolder.getMeta(Student.class), meta.getProfile(), null);
		meta.createIndex(index1);
		// 创建复合索引
		idef = new IndexDef("grade desc", "gender");
		Index index2 = Index.valueOf(idef, MetaHolder.getMeta(Student.class), meta.getProfile(), null);
